...
let uni = [];
uni = graduates.filter(school => school.university.indexOf("Uni")> -1);
console.log(uni);
...
zooAnimals.forEach(names => {
  return animalNames.push(names.animal_name + " "+ names.scientific_name)
});
...
let lowerCase = [];
lowerCase = zooAnimals.map(name => name.animal_name)
...
*/
let lowerPopulation = [];
lowerPopulation = zooAnimals.filter(number => number.population < 5);
...
populationTotal = zooAnimals.reduce((acc, animal) => {
  let numbers = animal.population;
  return acc + numbers;
}, 0);
